...
class TestCRMInvitation(FrappeTestCase):
	...
	def test_invitation_email_uses_configured_brand_name(self):
		frappe.db.set_single_value("FCRM Settings", "brand_name", "PEXEK")

		with patch.object(frappe, "sendmail") as sendmail:
			frappe.get_doc(
				doctype="CRM Invitation",
				email="pexek-teammate@example.com",
				role="Sales User",
			).insert(ignore_permissions=True)

		sendmail.assert_called_once()
		call = sendmail.call_args.kwargs
		self.assertEqual(call["subject"], "You have been invited to join PEXEK")
		self.assertEqual(call["args"]["title"], "PEXEK")
